...
% Recovery is where Unanimous BPaxos and Fast BPaxos differ. The correctness of
% recovery hinges on maintaining the following invariant:
...
instance $I_c$ with command $c$ that is not in $\deps{a}$. This is illustrated
in \figref{FastBPaxosNotUnion}. $Q$ proceeds as follows.
\begin{itemize}
  \item
    If $Q$ knows that $(I_c, c, \deps{c})$ has been chosen:
    \begin{itemize}
      \item
        If $I_a \notin \deps{c}$ (i.e., there is no edge from $I_c$ to $I_a$),
        then we can conclude that $(a, \deps{a})$ has not been chosen.
      \item
    \end{itemize}
  \item
    Otherwise, $Q$ recovers $I_c$.
\end{itemize}
