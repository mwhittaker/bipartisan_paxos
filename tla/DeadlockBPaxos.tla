...
\* TODO: Rethink Ploop and Pnoop.
Gadget == [
  cmd: Command \union {noop},
  deps: SUBSET Instance,
  Pnoop: SUBSET Instance,
  Ploop: SUBSET Instance
]
...
DepServiceProcess(d, I) ==
  ...
  /\ LET cmd == proposed[I]
         gadget == [cmd |-> cmd,
                    deps |-> Dependencies(G, cmd),
                    Pnoop |-> {},
                    Ploop |-> {}] IN
    /\ dependencyGraphs' = [dependencyGraphs EXCEPT ![d][I] = gadget]
...
IsProposable(Q, I, i, M, v) ==
  ...
  ELSE
    /\ ExistsQuorumReply(Q, I)
    /\ LET paxosGadget == CHOOSE w \in V : O4(w)
           depServiceGadget == QuorumReply(Q, I)
           wantToPrune == depServiceGadget.deps \ paxosGadget.deps IN
      \/ \E Ib \in wantToPrune :
         \E gadget \in chosen[Ib] :
         /\ gadget.cmd /= noop
         /\ I \notin gadget.deps
         /\ I \notin gadget.Ploop
         /\ AnyProposal(I, v)
      \/ \E Pnoop, Ploop \in SUBSET Instance :
         \A Ib \in wantToPrune :
         \E gadget \in chosen[Ib] :
         /\ \/ (gadget.cmd = noop /\ Ib \in Pnoop)
            \/ (I \in gadget.deps /\ Ib \in Ploop)
         /\ v = [cmd |-> paxosGadget.cmd,
                 deps |-> paxosGadget.deps,
                 Pnoop |-> Pnoop,
                 Ploop |-> Ploop]
...
\* TODO(mwhittaker): We should ignore Ploop and Pnoop when choosing. They are
\* not part of the value being chosen.
...
ConsensusConsistency ==
  \A I \in Instance :
    LET unprunedChosen == {<<g.cmd, g.deps>> : g \in chosen[I]} IN
    Cardinality(unprunedChosen) =< 1
