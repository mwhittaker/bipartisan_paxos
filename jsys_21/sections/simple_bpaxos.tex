...
\markrevisions{%
Note that if a client does not receive a response for its pending request for a
sufficiently long period of time, it resends its request. This means that if a
client's command is replaced by a noop, the client will eventually re-propose
the command.
}
