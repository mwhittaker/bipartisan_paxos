...
\begin{itemize}
  ...
  \item \textbf{(4) and (5)}
    ...
    $p_i$ executes round $0$ for vertices of the form $(p_i, m)$. This is why
...
\begin{itemize}
  ...
  \item
    In \figref{SimpleBPaxosExample3}, $p_1$ executed Phase 2 of Paxos to get
...
\TODO[mwhittaker]{Cite atomic broadcast stuff.}

\subsection{Recovery}
{\input{figures/simple_bpaxos_recovery_example.tex}}
...
execution is given in \figref{SimpleBPaxosRecoveryExample}.

\begin{itemize}
  \item
    In \figref{SimpleBPaxosRecoveryExample1}, proposer $p_1$ receives command
    $x$ from a client. It places $x$ in vertex $v_x$ and sends $x$ and $v_x$ to
    the dependency service. Shortly after, it fails.

  \item
    In \figref{SimpleBPaxosRecoveryExample2}, proposer $p_2$ receives command
  ...
  \item
    In \figref{SimpleBPaxosRecoveryExample3}, the replicas cannot execute
  ...
  \item
    In \figref{SimpleBPaxosRecoveryExample4}, $p_2$ executes Phase 1 and Phase
