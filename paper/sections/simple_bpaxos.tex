...
%
Clients sends state machine commands to BPaxos nodes for execution. When a
...
conflict graph. See \secref{ConflictGraphs}.} When $d_i$ receives the tuple
...
consensus service (not illustrated in \figref{SimpleBPaxosExample}). The
...
\figref{SimpleBPaxosExample3}.  They then both execute $x$ and $y$ in some
