...
Reiterating \secref{BipartisanPaxos}, to prove the correctness of Simple
...
\begin{invariant}\invlabel{SimpleBpaxosInvariant}
  For every instance $I$, a BPaxos node only proposes a value $(x, \deps{I})$
  in instance $I$ if $(I, x, \deps{I})$ is a response from the ordering service
  or if $(x, \deps{I}) = (\noop, \emptyset)$.
\end{invariant}

\invref{DependencyService} and \invref{SimpleBpaxosInvariant} trivially imply
\invref{ConflictInvariant}. Note that proposing $\noop{}$s does not affect
\invref{ConflictInvariant} because $\noop$s do not conflict with any other
command, so \invref{ConflictInvariant} holds vacuously.

\TODO[mwhittaker]{Highlight how modularity helps make this proof simple.}
\TODO[mwhittaker]{Mention that the protocol can be driven by clients.}
