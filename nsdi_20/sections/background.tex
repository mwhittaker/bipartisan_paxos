...
by the leader. As shown in \figref{MultiPaxosCommunication}, \emph{every}
...
in parallel. We call these protocols \defword{multileader} protocols.
...
log in \figref{ExampleMultiPaxosExecution} could execute commands $a \gets 0$
...
is \defword{generalized}.

% When we run with multiple leaders, we don't have a single spot to assign
% commands to log entries. This makes it difficult to establish a single total
% order. Thus, most leaderless protocols are generalized. You don't have to be.
% Mencius isn't but if you don't, then things start to run as fast as the
% slowest leader. All to all leader communication prevents thriftiness.
% Most multileader protocols are generalized~\cite{mao2008mencius,
% moraru2013there, zhang2018building, mu2016consolidating}.
