...
\begin{figure}[t]
  ...
  \caption{%
    ...
    %
    Note that this figure and \figref{MultiPaxosBottleneck} are drawn with a
    single client for simplicity. With $k$ clients, all $k$ commands are sent
    to the single MultiPaxos leader, while the $k$ commands are load balanced
    across the $L$ BPaxos leaders and proposers.
  }
