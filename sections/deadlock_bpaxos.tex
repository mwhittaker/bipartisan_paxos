...
\algoref{DeadlockBPaxos}. We'll walk through \algoref{DeadlockBPaxos}
momentarily. The main idea is that in order to avoid the upper right corner of
\figref{BPaxosLogic}, if a Deadlock BPaxos proposer thinks that a value $v$ may
  ...
  \begin{algorithmic}[1]
    ...
    \For{$I_b \in \deps{a}' - \deps{a}$}
      ...
      \If{$I_b$ committed with $\noop$}
        \State{} continue
      \EndIf{}
      \If{$I_b$ committed with $R.i \in I_b$}
        \Comment{$b \to a$}
        \State{} continue
      \EndIf{}
      \If{$I_b$ committed without $R.i \in I_b$}
        \Comment{$b \not\to a$}
        \State{} propose $(\noop, \emptyset)$
      \EndIf{}
    \EndFor{}
